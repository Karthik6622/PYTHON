        ...
        if (dic['milk'] != 0):
            ...
            if (dic['water'] != 0):
                ...
                if (dic['sugar'] != 0):
                    ...
                    if (dic['coffee'] != 0):
                        # here Explaning one latte rate
                        print("-" * 100)
                        print("\033[1;34m1 Latte rate is 15Rupees\033[0m")
                        print("-" * 100)
                        # one Latte need 100ml water,200ml milk and 25g sugar and 20g coffee
                        # here getting how much Latte user want
                        try:
                            count_latte = int(input("\033[1;35mSelect no of Latte do you want?:\033[0m"))
                        except:
                            print("Please Enter Proper Numbric Value")
                        else:
                            # checkcing if machine water level equal or morethan making latte for user need or not
                            # count_latte*0.100=total water need for customer request
                            if (dic['water'] >= count_latte * 0.100):
                                # checkcing if machine milk level equal or morethan making Latte for user need or not
                                # count_capacino*0.200=total milk need for customer request
                                if (dic['milk'] >= count_latte * 0.200):
                                    # checkcing if machine sugar level equal or morethan making Latte for user need or not
                                    # count_capacino*0.025=total sugar need for customer request
                                    if (dic['sugar'] >= (count_latte * 0.025)):
                                        # checkcing if machine Coffee Powder level equal or morethan making Latte for user need or not
                                        # count_capacino*0.020=total coffee need for customer request
                                        if (dic['coffee'] >= (count_latte * 0.020)):
                                            # here displaying Latte total rate
                                            print(
                                                f'\033[1;34mYou Have Selected {count_latte} Latte Rate is:{count_latte * 15}\033[0m')
                                            # getting money from user for capacino
                                            try:
                                                latte_money = float(input(
                                                    "\033[1;35mPlease Pay Money For Your Latte As Mentoned Above:\033[0m"))
                                            except:
                                                print("Please Enter Numeric Or Decimal Value")
                                            else:
                                                # checking user paying money is morethan payable money if it is equal or more it will go inside
                                                if (latte_money >= count_latte * 15):
                                                    # dic['money']=machine money
                                                    # here checking we have enough money to give user as a change after user paid morethan actul money
                                                    if (dic['money'] >= latte_money - count_latte * 15):
                                                        ...
                                                        dic['water'] = round(dic['water'] - 0.100 * count_latte, 3)
                                                        dic['milk'] = round(dic['milk'] - 0.200 * count_latte, 3)
                                                        dic['coffee'] = round(dic['coffee'] - 0.020 * count_latte, 3)
                                                        dic['money'] = round(dic['money'] + latte_money, 1)
                                                        print(
                                                            "\033[1;34mYour Order has been placed!!!please keep Cup near machine..\033[0m")
                                                        # here Filling latte to user's cup one by one using while loop
                                                        j = 1
                                                        else_part = 0
                                                        while (j <= count_latte):
                                                            temp_sugar = 0.0
                                                            ...
                                                            # if user selected high,we will add sugar 0.025g to user Latte
                                                            if (re.search('^high$|^HIGH$', press_sugar, re.IGNORECASE)):
                                                                temp_sugar = round(temp_sugar + 0.025, 3)
                                                            # if user selected Medium,we will add sugar 0.020g to user Latte
                                                            elif (
                                                            (re.search('^medium$|^MEDIUM$', press_sugar, re.IGNORECASE))):
                                                                temp_sugar = round(temp_sugar + 0.020, 3)
                                                            # if user selected Low,we will add sugar 0.015g to user Latte
                                                            elif ((re.search('^low$|^LOW$', press_sugar, re.IGNORECASE))):
                                                                temp_sugar = round(temp_sugar + 0.015, 3)
                                                            else:
                                                                # if user has not selected any sugar levels,we defaultly providing medium sugar level to user latte
                                                                print(
                                                                    "\033[1;31mYou Not Selected Proper Sugar Level so Machine provide defaulty Medium Sugar level!!!\033[0m")
                                                                temp_sugar = round(temp_sugar + 0.020, 3)
                                                                # This one Last confimation from the user to fill latte

                                                            press_ok = input(
                                                                "\033[1;35mEnter Ok To Fill Latte To Your Cup(If You Want to cancel your order please click cancel or other tahn ok):\033[0m")
                                                            print('*' * 100)

                                                            # once we get confirmation from user,we going to fill Latte
                                                            if (re.search('^ok$|^OK$', press_ok, re.IGNORECASE)):
                                                                # here we subtracting sugar that how much used for Latte
                                                                dic['sugar'] = round(dic['sugar'] - temp_sugar, 3)
                                                                # Showing filling process by progress bar
                                                                for i in tqdm(range(10), desc="Latte Is Filling",
                                                                              bar_format="\033[1;32m{l_bar}{bar}"):
                                                                    time.sleep(.2)
                                                                # once we given to user entered no of latte, we displaying order completed Message
                                                                if (j == count_latte):
                                                                    print(
                                                                        "\033[1;31mYour Order has been completed!!!Thankyou For Using Machine\033[1;35m")
                                                                # once we not given to user entered no of latte, we asking to keep next cup
                                                                else:
                                                                    print(
                                                                        f"\033[1;31mFilled Up!!! Please Keep Next Cup ...Still {count_latte - j} Cups Are Pending To Fill \033[0m")
                                                                j += 1
                                                            ...
                                                            # unfortunitly if user clicked other than Ok,we calculating in Which Cup filling time user stopped and that stuffs adding again to machine
                                                                ...
                                                                else_part = 1
                                                                dic['water'] = round(
                                                                    dic['water'] + (0.100 * (count_latte - j + 1)), 3)
                                                                dic['milk'] = round(
                                                                    dic['milk'] + (0.200 * (count_latte - j + 1)), 3)
                                                                dic['coffee'] = round(
                                                                    dic['coffee'] + (0.020 * (count_latte - j + 1)), 3)
                                                                # 15*count_latte-j*15+15(j=2,count_latte=5)
                                                                # 15*5-2*15+15
                                                                # 75-30+15
                                                                # 45+15
                                                                # 60
                                                                print(
                                                                    f"\033[1;3mYOU CLICKED OTHER THAN OK!!So Your Money {15 * count_latte - j * 15 + 15}Rupees be there to give\033[0m")
                                                                print(
                                                                    f"Cause You ordered {count_latte} Capacino but you cancelled before {count_latte - j + 1} cup filling so we providig that money")
                                                                dic['money'] = round(
                                                                    dic['money'] - (15 * count_latte - j * 15 + 15), 1)
                                                                if (latte_money >= count_latte * 15):
                                                                    change = 0
                                                                    change = round(latte_money - count_latte * 15 + (
                                                                                15 * count_latte - j * 15 + 15), 1)
                                                                    print("\033[1;31myour Change Is:",
                                                                          round(latte_money - count_latte * 15, 1),
                                                                          "Rupees\033[0m")
                                                                    # after given change subtracting money from machine money
                                                                    dic['money'] = round(
                                                                        dic['money'] - (latte_money - count_latte * 15), 1)
                                                                print("Plese Collect Your Total Amount:", change)
                                                                j = count_latte + 1

                                                        # here we calculating how much we must give change to user
                                                        if (latte_money > count_latte * 15 and else_part != 1):
                                                            print("\033[1;31mPlease collect your Change:",
                                                                  round(latte_money - count_latte * 15, 1), "Rupees\033[0m")
                                                            # after given change subtracting money from machine money
                                                            dic['money'] = round(
                                                                dic['money'] - (latte_money - count_latte * 15), 1)
                                                    else:
                                            ...
                                            available_coffee = round(dic['coffee'] / 0.020)
